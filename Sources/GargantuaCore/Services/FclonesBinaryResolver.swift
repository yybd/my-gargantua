...
///    (`Bundle.gargantuaCoreResources/bin/fclones`). For a shipped `.app`, this lives
...
public struct FclonesBinaryResolver: Sendable {
    ...
    public static func defaultBundledURL() -> URL? {
        if let url = Bundle.gargantuaCoreResources.url(forResource: "fclones", withExtension: nil, subdirectory: "bin") {
            return url
        }
