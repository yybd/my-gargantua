...
struct MLXBackendPerformanceSmokeTests {
    ...
    @Test("Records cold load, warm generate latency, resident memory, and output tokens",
          .disabled(if: !smokeIsEnabled))
    func recordsBackendEnvelope() async throws {
        let modelDirectory = try resolveSmokeModelDirectory()
        let runsPerCase = smokeRunsPerCase
        let maxNewTokens = smokeMaxNewTokens
        ...
        let (_, coldLoadSeconds) = try await measureSeconds {
        ...
        let cases = representativeSmokeCases()
        ...
        cold_load_s=\(formatSeconds(coldLoadSeconds)) \
        memory_bytes=\(memoryAfterLoad) \
        memory_mib=\(formatMiB(memoryAfterLoad))
        """)
        ...
        for smokeCase in cases {
            ...
            for _ in 0 ..< runsPerCase {
                let (text, generateSeconds) = try await measureSeconds {
            ...
            p50_s=\(formatSeconds(percentile(latencies, 0.50))) \
            p95_s=\(formatSeconds(percentile(latencies, 0.95))) \
        ...
        print("""
        MLX_PERF_TOTAL \
        samples=\(allLatencies.count) \
        p50_s=\(formatSeconds(percentile(allLatencies, 0.50))) \
        p95_s=\(formatSeconds(percentile(allLatencies, 0.95)))
        """)
...
private func makeSmokeCase(_ spec: SmokeCaseSpec) -> SmokeCase {
    let rule = ScanRule(
        id: spec.id,
        name: spec.name,
        paths: [spec.path],
...
private func representativeSmokeCases() -> [SmokeCase] {
    [
        makeSmokeCase(SmokeCaseSpec(
            id: "chrome_cache",
