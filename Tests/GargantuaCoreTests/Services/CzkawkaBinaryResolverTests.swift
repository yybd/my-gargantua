...
@Suite("CzkawkaBinaryResolver")
struct CzkawkaBinaryResolverTests {

    private static func makeScratchBinary(executable: Bool = true) throws -> URL {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("CzkawkaResolverTests-\(UUID().uuidString)", isDirectory: true)
    ...
    func envVarOverride() throws {
        ...
        let resolver = CzkawkaBinaryResolver(
            environment: [CzkawkaBinaryResolver.envVarName: binary.path],
            bundledURL: nil
        )
    ...
    func envVarMissing() {
        let resolver = CzkawkaBinaryResolver(
            environment: [CzkawkaBinaryResolver.envVarName: "/definitely/not/a/real/path/czkawka_cli"],
            bundledURL: nil
        )

        #expect(throws: CzkawkaBinaryResolver.ResolutionError.notFound) {
    ...
    func envVarNotExecutable() throws {
        ...
        #expect(throws: CzkawkaBinaryResolver.ResolutionError.notExecutable(path: binary.path)) {
            _ = try resolver.resolve()
        }
    }
    ...
    func nothingAvailable() {
        // Skip if the host actually has czkawka_cli installed.
        let haveSystemBinary = CzkawkaBinaryResolver.candidatePaths.contains {
